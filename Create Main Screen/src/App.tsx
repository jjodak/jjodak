...
import { ChangeSchoolPage } from './components/ChangeSchoolPage';
...
export default function App() {
  ...
  // 로그인 상태 (null이면 로그인 안됨, 문자열이면 사용자 이메일)
  const [user, setUser] = useState<string | null>(null);
  
  // 인증 페이지 상태 ('login' | 'signup' | null)
  const [authPage, setAuthPage] = useState<'login' | 'signup' | null>('login');
  ...
  const [currentPage, setCurrentPage] = useState<
    'home' | 'school-select' | 'chatbot' | 'community' | 'post-detail' | 'write-post' | 
    'profile' | 'edit-profile' | 'change-school' | 'help' | 'privacy'
  >('home');
  ...
  const handleSignup = (userData: any) => {
    ...
    setSelectedSchool(userData.school); // 회원가입 시 선택한 학교 저장
    setAuthPage(null);
    alert(`회원가입이 완료되었습니다! 환영합니다, ${userData.name}님!`);
  };
  ...
  const handleLogout = () => {
    ...
    setCurrentPage('home');
    setActiveTab('home');
    setSelectedSchool(''); // 로그아웃 시 학교 정보도 초기화
  };
  ...
  const handleDeleteAccount = () => {
    ...
    setCurrentPage('home');
    setActiveTab('home');
    setSelectedSchool(''); // 탈퇴 시 학교 정보도 초기화
    alert('계정이 탈퇴되었습니다.');
  };
  ...
  const handleProfileNavigate = (page: string) => {
    ...
    } else if (page === 'change-school') {
      setCurrentPage('change-school');
    } else if (page === 'help') {
      setCurrentPage('help');
   ...
   * @param email - 새로운 이메일
   */
  const handleEditProfileSave = (name: string, email: string) => {
    setUser(email);
    setCurrentPage('profile');
    alert('프로필이 수정되었습니다.');
  };
   ...
   * @param school - 새로운 학교
   */
  const handleChangeSchoolSave = (school: string) => {
    setSelectedSchool(school);
    setCurrentPage('profile');
    alert(`학교가 "${school}"(으)로 변경되었습니다.`);
  };
  ...
  return (
    <div className="app-container">
      ...
      {/* 학교 변경 화면 */}
      {currentPage === 'change-school' && (
        <ChangeSchoolPage 
          currentSchool={selectedSchool}
          onBack={() => setCurrentPage('profile')}
          onSave={handleChangeSchoolSave}
        />
      )}
